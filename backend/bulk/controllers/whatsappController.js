...
const { sendTemplateMessage, sendTextMessage, uploadWhatsAppMedia } = require('../services/whatsappService');
...
async function getInbox(req, res) { try { const messages = await WhatsAppMessage.find({ conversationKey: { $ne: '' } }).sort({ createdAt: -1 }).limit(400).lean(); const grouped = new Map(); for (const item of messages) { const key = item.conversationKey || getConversationKey(item.from || item.to); if (!key) continue; const current = grouped.get(key); if (!current) grouped.set(key, { conversationKey: key, phone: key, contactName: item.contactName || '', lastMessage: item.bodyText || item.templateName || item.messageType, lastMessageAt: item.createdAt, lastDirection: item.direction, unreadCount: item.direction === 'INCOMING' && item.status !== 'READ' ? 1 : 0, lastStatus: item.status, messages: 1 }); else { current.unreadCount += item.direction === 'INCOMING' && item.status !== 'READ' ? 1 : 0; current.messages += 1; if (!current.contactName && item.contactName) current.contactName = item.contactName; } } res.json(Array.from(grouped.values()).sort((a, b) => new Date(b.lastMessageAt) - new Date(a.lastMessageAt))); } catch (err) { res.status(500).json({ message: err.message }); } }
async function getConversation(req, res) { try { const conversationKey = getConversationKey(req.params.conversationKey); const rows = await WhatsAppMessage.find({ conversationKey }).sort({ createdAt: 1 }).lean(); res.json(rows); } catch (err) { res.status(500).json({ message: err.message }); } }
async function markConversationRead(req, res) { try { const conversationKey = getConversationKey(req.params.conversationKey); await WhatsAppMessage.updateMany({ conversationKey, direction: 'INCOMING', status: { $in: ['RECEIVED', 'DELIVERED'] } }, { $set: { status: 'READ' } }); res.json({ message: 'Conversation marked as read' }); } catch (err) { res.status(500).json({ message: err.message }); } }
...
async function getRecipients(req, res) {
  try {
    const users = await User.find({ isActive: true }, "name mobile").sort({ name: 1 }).lean();
    res.json({ users: uniqueRecipients(users.map((u) => ({ name: u.name, mobile: u.mobile, source: "USER" }))) });
  } catch (err) { res.status(500).json({ message: err.message }); }
}
async function sendInvitation(req, res) { const { imageUrl, message = '', eventName = '', date = '', time = '', venue = '', textPosition, recipients = [], includeRsvp = false, rsvpYesLabel = 'Yes, I\'ll attend ✅', rsvpNoLabel = 'Sorry, can\'t make it ❌' } = req.body; if (!String(imageUrl || '').trim()) return res.status(400).json({ message: 'imageUrl is required' }); const cleanRecipients = uniqueRecipients((Array.isArray(recipients) ? recipients : []).map((item) => ({ name: String(item?.name || item?.fullName || 'Guest').trim() || 'Guest', mobile: item?.mobile || item?.phone || item?.number || item?.whatsapp, source: item?.source || 'CUSTOM' }))).filter((item) => String(item.mobile || '').length >= 10); if (!cleanRecipients.length) return res.status(400).json({ message: 'At least one valid recipient is required' }); const results = []; for (const recipient of cleanRecipients) { let finalImage = imageUrl; let uploadedMediaId = ''; try { finalImage = generateImageWithName(imageUrl, String(recipient.name || '').trim(), textPosition); const mediaUpload = await uploadWhatsAppMedia({ fileUrl: finalImage }); uploadedMediaId = mediaUpload?.id || ''; const providerResponse = await sendTemplateMessage({ to: recipient.mobile, templateName: 'entry_pass', languageCode: 'en_US', headerImageId: uploadedMediaId, bodyParameters: [String(recipient.name || '').trim(), String(eventName || '').trim(), String(date || '').trim(), String(time || '').trim(), String(venue || '').trim()] }); await createOutboundLog({ to: recipient.mobile, contactName: recipient.name, templateName: 'entry_pass', messageType: 'TEMPLATE', status: providerResponse?.messages?.length ? 'SENT' : 'QUEUED', source: 'MANUAL', providerResponse: { ...providerResponse, finalImage, uploadedMediaId } }); if (includeRsvp) { try { const rsvpText = `Please confirm your attendance:\n\n✅ Reply *${rsvpYesLabel}*\n❌ Reply *${rsvpNoLabel}*`; const rsvpResponse = await sendTextMessage({ to: recipient.mobile, body: rsvpText }); await createOutboundLog({ to: recipient.mobile, contactName: recipient.name, bodyText: rsvpText, messageType: 'TEXT', status: rsvpResponse?.messages?.length ? 'SENT' : 'QUEUED', source: 'INVITATION', providerResponse: rsvpResponse }); } catch (rsvpErr) { logger.warn('[whatsapp] RSVP text failed for', recipient.mobile, rsvpErr.message); } } results.push({ mobile: recipient.mobile, name: recipient.name, success: true }); } catch (error) { await createOutboundLog({ to: recipient.mobile, contactName: recipient.name, templateName: 'entry_pass', messageType: 'TEMPLATE', status: 'FAILED', source: 'MANUAL', providerResponse: { error: error?.response?.data || error.message, finalImage, uploadedMediaId } }); results.push({ mobile: recipient.mobile, name: recipient.name, success: false, error: error?.response?.data?.error?.message || error.message }); } } emitEvent('whatsapp_invitation_sent', { total: results.length, success: results.filter((item) => item.success).length }); res.json({ total: results.length, success: results.filter((item) => item.success).length, failed: results.filter((item) => !item.success).length, results }); }
async function verifyWebhook(req, res) { const mode = req.query['hub.mode']; const token = req.query['hub.verify_token']; const challenge = req.query['hub.challenge']; if (mode === 'subscribe' && token === getWebhookVerifyToken()) return res.status(200).send(challenge); return res.sendStatus(403); }
async function receiveWebhook(req, res) { try { const entries = Array.isArray(req.body?.entry) ? req.body.entry : []; for (const entry of entries) { for (const change of entry?.changes || []) { const value = change?.value || {}; const contactMap = {}; (value.contacts || []).forEach((contact) => { const waId = normalizePhone(contact?.wa_id); if (waId) contactMap[waId] = contact?.profile?.name || ''; }); for (const message of value.messages || []) await saveIncomingMessage(message, contactMap); for (const statusItem of value.statuses || []) { if (!statusItem?.id) continue; await WhatsAppMessage.findOneAndUpdate({ waMessageId: statusItem.id }, { $set: { status: String(statusItem.status || '').toUpperCase() || 'SENT', meta: statusItem || {} } }); } } } res.sendStatus(200); } catch (error) { logger.error('receiveWebhook error', error?.response?.data || error.message || error); res.sendStatus(500); } }
// getGroups listed WhatsApp Web groups from the unofficial client. The Cloud
// API has no group messaging, so the capability is gone rather than stubbed.
async function getGroups(_req, res) {
  return res.status(501).json({
    success: false,
    message: 'Group messaging is not available on the WhatsApp Cloud API.',
  });
}
...
async function handleMagicLinkTriggers(incomingMessage) {
  ...
  if (!user) {
    ...
    try {
      ...
      logger.error('[magic-link] send error:', e.message);
    }
    return true;
  }
  ...
  try {
    ...
    logger.error('[magic-link] error generating or sending link:', e.message);
  }
  return true;
}
