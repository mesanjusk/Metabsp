...
|---|---|---|
| Multi-tenancy / data isolation | 90 | `Organization`/`tenantId` model live (not vestigial, unlike the original audit finding); per-number uniqueness enforced at DB + app layer; `nextjs/lib/models/whatsappAccount.ts` |
...
| Secrets & encryption | 90 | AES-256-GCM token encryption with key-versioning (`nextjs/lib/utils/crypto.ts`, `crypto.test.js`); no plaintext secrets in code (only in the *dev* `docker-compose.yml`, which is explicitly labeled dev-only per `docs/deployment/DOCKER_DEPLOYMENT.md`) |
...
| Backup & disaster recovery | 85 | `nextjs/scripts/backup-mongo.sh`/`restore-mongo.sh`, optional in-process scheduler, `verify-restore.js` drill script, full runbook in `docs/deployment/DISASTER_RECOVERY.md`; RPO is ~24h on cron-only cadence (documented, not hidden) |
...
These are **qualitative, reasoned estimates**, not measured benchmarks — this repo has no load-test results run against real production infrastructure to cite (see `nextjs/loadtest/README.md`'s own honesty note on this). Treat as planning input, not a guarantee.
...
12. Run the load-testing scripts (`nextjs/loadtest/`) against real target infrastructure to replace this report's qualitative estimates with measured numbers.
