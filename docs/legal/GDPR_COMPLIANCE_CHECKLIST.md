...
| Item | Status | Basis |
|---|---|---|
| Encryption of personal data | **Implemented (tokens only)** | WhatsApp access tokens are encrypted at rest with AES-256-GCM and support key rotation (`nextjs/lib/utils/crypto.ts`). Message/contact data itself is stored in MongoDB without field-level application encryption beyond whatever the hosting/database layer provides at rest — **operator should confirm and document disk/volume-level encryption at the infrastructure layer** if message/contact content encryption at rest is being represented as a control. |
