...
export async function requireAuth(req: NextRequest): Promise<AuthedUser> {
  ...
  try { decoded = verifyToken(token); } catch (_error) { throw new AppError('Invalid token', 401); }
  ...
  const authed: AuthedUser = { id: String(user._id), isAdmin: permissions.includes('*'), tenantId: user.tenantId || null, doc: user };
