  ...
  return withJobLifecycle(bullJob, async (jobDoc) => {
    ...
    const compose = await renderer.render({
      scenes: renderable.map((s) => {
        // Two ways a clip can already speak, and they are treated the same.
        //
        // A lip-synced clip has the narration baked in — that was always true. A clip generated by
        // Google Flow has its own sound too (see Scene#videoHasAudio), which is why no voice track
        // was ever made for it. Either way the separate track is left unused, because mixing a
        // synthesised reading over a performance the clip already contains is worse than either.
        const lipSynced = !!(s.lipSyncAssetId && typeof s.lipSyncAssetId === "object");
        const speaksForItself = lipSynced || !!s.videoHasAudio;
        return {
          index: s.index,
          videoUrl: lipSynced
          ...
          voiceUrl:
            !speaksForItself && s.voiceAssetId && typeof s.voiceAssetId === "object"
              ? (s.voiceAssetId as unknown as { url: string }).url
              : undefined,
          useEmbeddedAudio: speaksForItself,
          dialogue: s.dialogue,
        };
      }),
