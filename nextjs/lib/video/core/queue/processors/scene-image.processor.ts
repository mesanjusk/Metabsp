...
import { Background } from "@/lib/video/modules/backgrounds/models/Background";
...
import { sceneImagePrompt, sceneImageReferences } from "@/lib/video/core/ai/providers/image-prompts";
...
export async function processSceneImageJob(bullJob: BullJob<BullJobData>): Promise<ProcessorResult> {
  return withJobLifecycle(bullJob, async (jobDoc) => {
    ...
    const image = await routeSingleImage(jobDoc, {
      ...
      prompt: sceneImagePrompt(input),
      referenceUrls: sceneImageReferences(input),
      viaApi: (provider) => provider.generateSceneImage(input, context),
      flow: {
