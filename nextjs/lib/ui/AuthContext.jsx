...
//
// Two changes were needed for the App Router. `getInitialUser` reads
// localStorage, and useState(initialiser) runs during the server render too —
// where there is no localStorage. The storage helpers already return '' in
// that case, so the server renders a signed-out shell and the first client
// render fills in the real session. That is a deliberate hydration difference:
// rendering the signed-in state on the server would mean shipping one user's
// session in cached HTML.
//
// Everything else is unchanged from the Vite version, including the somewhat
// odd `[user?.userGroup]` dependency lists, which are preserved rather than
// "fixed" so behaviour cannot drift between the two apps while both are live.
