...
/**
 * Renders an invoice to a PDF buffer. Deliberately plain — line items and the
 * legal minimum, not final visual design, since a logo would have to be
 * supplied per deployment.
 */
export function renderInvoicePdf({ invoice, organization }: { invoice: any; organization: any }): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    const asDate = (value: any) => new Date(value).toISOString().slice(0, 10);
    doc.fontSize(11).text(`Billing period: ${asDate(invoice.periodStart)} to ${asDate(invoice.periodEnd)}`);
    doc.moveDown(1);
