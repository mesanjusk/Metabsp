...
async function list(userId: any, match: Record<string, any>, limit = 8) {
  return SmbRecord.find({ userId, ...match }).sort({ dueAt: 1, updatedAt: -1 }).limit(limit)
    .populate('contactId', 'name phone category assignedAgent').lean();
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (/follow.?up|call back|callback/.test(q)) {
      ...
      await requireSmbKindAccess(authed, 'payment');
      mode = 'outstanding';
      ...
      await requireSmbKindAccess(authed, 'task');
      mode = 'tasks';
      ...
      await requireSmbKindAccess(authed, 'expense');
      mode = 'expenses';
      ...
      if (canPayments) {
        ...
        answer = `This month you have ${money(sales)} in recorded orders. Collections require Payments access.`;
      }
      ...
      const [leads, followups, orders, tasks, outstanding] = await Promise.all([
        ...
        canStaff ? SmbRecord.countDocuments({ userId, kind: 'task', status: { $nin: CLOSED }, dueAt: { $lte: now } }) : Promise.resolve(0),
        canPayments ? sum(userId, { kind: 'order', status: { $nin: ['cancelled', 'lost', 'rejected'] }, balanceInPaise: { $gt: 0 } }, 'balanceInPaise') : Promise.resolve(0),
      ]);
      answer = `Right now: ${leads} open leads, ${followups} due follow-ups and ${orders} open orders${canStaff ? `, ${tasks} due tasks` : ''}${canPayments ? ` and ${money(outstanding)} outstanding` : ''}.`;
    }
