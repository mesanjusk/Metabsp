...
import { verifyGoogleIdToken } from '@/lib/services/socialAuthService';
import { recordAuditEvent } from '@/lib/services/auditLogService';
import logger from '@/lib/utils/logger';

// Ported from backend/src/routes/Users.js's POST /auth/google.
...
export async function POST(req: NextRequest) {
  ...
  const allowed = await checkAuthRateLimit(req, { windowMs: 15 * 60 * 1000, maxRequests: 20, scope: 'auth-google' });
  if (!allowed) {
  ...
  try {
    ...
    const profile = await verifyGoogleIdToken(String(body?.credential || body?.idToken || ''));
    return await completeSocialLogin({ req, profile, provider: 'google' });
  } catch (error: any) {
    const status =
